...
/**
 * Google Gemini 大模型Provider实现<br/>
 * 使用 OpenAI 兼容模式（/v1beta/openai/chat/completions）<br/>
 * API文档：<a href="https://ai.google.dev/gemini-api/docs/openai">...</a><br/>
 * <p>
 * API Key 管理页面<br/>
 * <a href="https://aistudio.google.com/app/apikey">...</a>
 * <p>
 * 注意：Gemini 使用 x-goog-api-key 请求头进行认证，而非标准 Bearer Token。
 *
 * @author 百岁 (baisui@qlangtech.com)
 * @date 2026/8/30
 */
public class GeminiLLMProvider extends LLMProvider {
    private static final Logger logger = LoggerFactory.getLogger(GeminiLLMProvider.class);

    private static final String URL_PATH = "/v1beta/openai/chat/completions";
    ...
    /**
     * Gemini 使用 x-goog-api-key 请求头进行认证，而非标准 Bearer Token
     */
    @Override
    protected List<ConfigFileContext.Header> appendHeaders() {
        return List.of(new ConfigFileContext.Header("x-goog-api-key", getApiKey()));
    }
    ...
    public static final class DftDescriptor extends BasicParamsConfigDescriptor implements IEndTypeGetter {
        ...
        protected boolean verify(IControlMsgHandler msgHandler, Context context, PostFormVals postFormVals) {
            GeminiLLMProvider provider = postFormVals.newInstance();
            try {
